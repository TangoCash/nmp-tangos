		...
		else if (msg == CRCInput::RC_down || (int) msg == g_settings.key_channelList_pagedown)
		{
			...
			if(CNeutrinoApp::getInstance()->getMode() != NeutrinoMessages::mode_ts) {
				switch (g_settings.channellist_new_zap_mode) {
					case 2:
						g_settings.channellist_new_zap_mode = 1;
						break;
					case 1:
						g_settings.channellist_new_zap_mode = 2;
						break;
					default:
						break;
...
#endif
	::paintButtons(x, y_foot, full_width, num_buttons, Button, width, footerHeight);
}
...
void CChannelList::paint()
{
	if (g_settings.channellist_additional == 2) // with miniTV
	{
		...
		if(CNeutrinoApp::getInstance()->getMode() == NeutrinoMessages::mode_tv) {
			...
			g_PicViewer->DisplayImage(DATADIR "/neutrino/icons/radiomode.jpg", x+width+5, y+theight+5, pig_width-10, pig_height-10);
		}
	}
