...
void CEpgData::showText( int startPos, int ypos )
{
	...
	for(int i = 0; i < 12;i++){
		max_mon_w = std::max(max_mon_w ,g_Font[SNeutrinoSettings::FONT_TYPE_EPG_INFO2]->getRenderWidth(std::string(g_Locale->getText(CLocaleManager::
getMonth(i))) + " ", true)); // UTF-8
		if(i > 6)
		      continue;
